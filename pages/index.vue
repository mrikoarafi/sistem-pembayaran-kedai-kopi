  ...
  <div class="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50">
          ...
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center">
              <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <path
                  d="M3 4a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 15.846 4.632 18 6.414 18H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 8H6.28l-.31-1.243A1 1 0 005 6H3a1 1 0 000-2z"
                />
              </svg>
            </div>
            <div>
          ...
          <div class="flex items-center gap-4">
            ...
            >
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path
                  d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 15.846 4.632 18 6.414 18H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 8H6.28l-.31-1.243A1 1 0 005 6H3a1 1 0 000-2z"
                />
              </svg>
              <span
          ...
          <div class="p-6">
            ...
            <div class="mb-4">
              <div class="flex items-baseline gap-2 mb-2">
                ...
                <span v-else class="text-2xl font-bold text-gray-800"> Rp {{ formatCurrency(produk.harga) }} </span>

                <span v-if="produk.diskon > 0" class="text-lg text-gray-400 line-through"> Rp {{ formatCurrency(produk.harga) }} </span>
              </div>
            ...
            <div class="flex justify-between items-center text-sm mb-4">
              ...
              <span :class="['font-medium', produk.stok > 10 ? 'text-green-600' : produk.stok > 0 ? 'text-yellow-600' : 'text-red-600']">
                {{ produk.stok }} porsi
              </span>
            </div>
            ...
            <div class="text-center text-sm font-medium">
              <span v-if="produk.stok === 0" class="text-gray-400"> Stok tidak tersedia </span>
              <span v-else-if="getCartQuantity(produk.id) > 0" class="text-amber-600"> Tap untuk ubah kuantitas </span>
              <span v-else class="text-amber-600"> Tap untuk memilih kuantitas </span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-t-2xl md:rounded-2xl w-full md:max-w-2xl max-h-90vh overflow-hidden">
        ...
        <div class="max-h-96 overflow-y-auto p-6">
          ...
          <div v-else class="space-y-4">
            <div v-for="item in cart" :key="item.id" class="flex items-center gap-4 p-4 border rounded-lg">
              <img v-if="item.produk.image" :src="item.produk.image" :alt="item.produk.nama" class="w-16 h-16 object-cover rounded-lg" />
    ...
    <div v-if="showSuccessModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl text-center">
        ...
        <p class="text-sm text-gray-500 mb-6">Total: Rp {{ formatCurrency(orderTotal) }}</p>
        <button
...
const filteredProduk = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(
      (produk) => produk.nama.toLowerCase().includes(query) || (produk.deskripsi && produk.deskripsi.toLowerCase().includes(query))
    );
  }
